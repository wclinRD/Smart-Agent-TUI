...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("smart://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("smart://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("smart://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("smart://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("smart://open-project")).toBeUndefined()
    expect(parseDeepLink("smart://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "smart://open-project?directory=/a",
      "smart://other?directory=/b",
      "smart://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("smart://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("smart://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("smart://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("smart://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "smart://new-session?directory=/a",
      "smart://open-project?directory=/b",
      "smart://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __SMART__: {
        deepLinks: ["smart://open-project?directory=/a"],
      },
    } as unknown as Window & { __SMART__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["smart://open-project?directory=/a"])
