  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://smart.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://smart.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://smart.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          ...
          lastProject: { local: "/local", "https://smart.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://smart.example.com"),
      ),
    ).toEqual({
