...
import { Spinner } from "@smartcode-ai/ui/spinner"
import { showToast } from "@smartcode-ai/ui/toast"
...
type WslServerStep = "wsl" | "distro" | "smart"

const STEPS: WslServerStep[] = ["wsl", "distro", "smart"]
...
export function DialogAddWslServer(props: DialogWslServerProps = {}) {
  ...
  const smartCheck = createMemo(() => {
    const distro = selectedDistro()
    if (!distro) return null
    return current()?.smartChecks[distro] ?? null
  })
  ...
  const installingOpencode = createMemo(() => {
    ...
    return job?.kind === "install-smart" && job.distro === selectedDistro()
  })
  const allReady = createMemo(() => wslReady() && distroReady() && smartReady())
  const addDisabled = createMemo(() => {
    const job = current()?.job
    if (!job) return store.adding
    return store.adding || job.kind !== "probe-smart"
  })
  ...
  const autoProbe = createMemo(() => {
    ...
    if (!state.smartChecks[distro]) {
      return { key: `probe-smart:${distro}`, run: () => api.probeOpencode(distro) }
    }
    return null
  })
  ...
  const smartMessage = createMemo(() => {
    ...
    if (smartReady()) {
      return distro
        ? language.t("wsl.onboarding.smartReadyIn", { distro })
        : language.t("wsl.onboarding.smartReady")
    }
    return distro
        ...
        title:
          step === "wsl"
            ...
            : step === "distro"
              ...
              : language.t("wsl.onboarding.step.smart"),
        state:
          active === step
            ? "current"
            : step === "wsl"
              ...
              : step === "distro"
                ? distroReady()
                  ...
                  : index > activeIndex
                    ? "locked"
                    : "warning"
                : smartCheck()?.matchesDesktop === false
                  ? "warning"
                  : smartReady()
                    ? "done"
                    : index > activeIndex
      ...
      >
        ...
        >
          ...
          <Switch>
            ...
            <Match when={activeStep() === "smart"}>
              <div class="rounded-md bg-surface-base p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-14-medium text-text-strong">{language.t("wsl.onboarding.step.smart")}</div>
                  <div class="flex items-center gap-2">
                    <Show when={selectedDistro()}>
                      <Button
                        variant="ghost"
                    ...
                    <Show when={!smartReady() || smartCheck()?.matchesDesktop === false}>
                      <Button
                        variant="secondary"
                        size="large"
                        disabled={busy()}
                ...
                <div class="text-12-regular text-text-weak whitespace-pre-wrap break-words">{smartMessage()}</div>
                <Show when={smartCheck()?.matchesDesktop === false ? smartCheck() : null}>
                  {(check) => (
                    <div class="rounded-md border border-border-weak-base px-3 py-3 flex flex-col gap-1">
                      <div class="text-12-regular text-text-weak">
          ...
          <Show when={activeStep() === "smart" && allReady() && selectedDistro()}>
