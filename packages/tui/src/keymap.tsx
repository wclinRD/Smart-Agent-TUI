...
export const SMART_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const SMART_MODE_KEY = "smart.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(SMART_MODE_KEY, SMART_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(SMART_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(SMART_MODE_KEY, stack.at(-1)?.mode ?? SMART_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? SMART_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(SMART_MODE_KEY, undefined)
