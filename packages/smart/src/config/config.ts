import { LayerNode } from "@smart-ai/core/effect/layer-node"
import { httpClient } from "@smart-ai/core/effect/layer-node-platform"
import { serviceUse } from "@smart-ai/core/effect/service-use"
...
import { InstallationLocal, InstallationVersion } from "@smart-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@smart-ai/core/v1/config/console-state"
...
import { RemoteAuthError } from "@smart-ai/core/v1/config/error"
import { ConfigPermissionV1 } from "@smart-ai/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@smart/Config") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://smart.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.SMART_CONFIG && !Flag.SMART_CONFIG_DIR && !Flag.SMART_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://smart.ai/config.json" }, null, 2))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://smart.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "SMART_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/smart`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://smart.ai/config.json"
        ...
        if (Flag.SMART_CONFIG) {
          yield* merge(Flag.SMART_CONFIG, yield* loadFile(Flag.SMART_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.SMART_CONFIG })
        }

        if (!Flag.SMART_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("smart", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.SMART_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from SMART_CONFIG_DIR", { path: Flag.SMART_CONFIG_DIR })
        }
        ...
        for (const dir of directories) {
          ...
          // Auto-discovered plugins under `.smart/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.SMART_CONFIG_CONTENT) {
          const source = "SMART_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.SMART_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from SMART_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["SMART_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("SMART_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.SMART_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.SMART_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("SMART_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.SMART_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.SMART_DISABLE_PRUNE) {
