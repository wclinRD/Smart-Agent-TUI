...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for smart serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(SmartHttpApi)
...
export const PublicApi = SmartHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "smart",
    version: "1.0.0",
