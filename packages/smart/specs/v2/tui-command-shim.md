...
Delete `packages/smart/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/smart/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/smart/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/smart`
- TUI plugin loader tests in `packages/smart` if runtime plugin API wiring changed
