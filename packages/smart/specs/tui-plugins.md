...
- Author package entrypoint is `@smart-ai/plugin/tui`.
...
- Import types from `@smart-ai/plugin/tui`.
- `@smart-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@smart-ai/plugin/tui"
...
- `smart plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `smart plug <module>`.
...
When `plugin` entries exist in a writable `.smart` dir or `SMART_CONFIG_DIR`, Smart installs `@smart-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@smart-ai/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@smart-ai/plugin/tui`.
