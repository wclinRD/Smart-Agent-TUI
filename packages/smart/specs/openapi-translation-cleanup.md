...
Trim `packages/smart/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/smart/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/smart`.
- `[x]` `bun typecheck` from `packages/smart`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/smart/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/smart`.
- `[x]` `bun dev generate > /tmp/smart-openapi.json` from `packages/smart`.
...
- Edit `packages/smart/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/smart-openapi.json` from `packages/smart`.
...
- Check source schemas in files like `packages/smart/src/session/schema.ts`, `packages/smart/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/smart/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/smart/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
