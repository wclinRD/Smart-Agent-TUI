...
- The server still lives in `packages/smart`.
...
Until enough shared service code lives outside `packages/smart`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/smart`

It should not import `packages/smart` services while `packages/smart`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/smart` implementation details.
