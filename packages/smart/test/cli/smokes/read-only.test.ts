...
describe("smart read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // SMART_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ smart }) =>
      Effect.gen(function* () {
        const r = yield* smart.spawn(["mcp", "list"])
        smart.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ smart }) =>
      Effect.gen(function* () {
        const r = yield* smart.spawn(["providers", "list"])
        smart.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ smart }) =>
      Effect.gen(function* () {
        const r = yield* smart.spawn(["models"])
        smart.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ smart }) =>
      Effect.gen(function* () {
        const r = yield* smart.spawn(["agent", "list"])
        smart.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh SMART_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ smart }) =>
      Effect.gen(function* () {
        const r = yield* smart.spawn(["session", "list"])
        smart.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ smart }) =>
      Effect.gen(function* () {
        const r = yield* smart.spawn(["stats"])
        smart.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under SMART_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ smart }) =>
      Effect.gen(function* () {
        const r = yield* smart.spawn(["db", "path"])
        smart.expectExit(r, 0, "db path")
