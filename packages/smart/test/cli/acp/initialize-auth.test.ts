...
describe("smart acp initialize/auth subprocess", () => {
  cliIt.live(
    ...
    ({ smart }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.agentInfo?.name).toBe("Smart")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ smart }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ smart })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("smart-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "smart-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.SMART_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ smart }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("smart-login")
