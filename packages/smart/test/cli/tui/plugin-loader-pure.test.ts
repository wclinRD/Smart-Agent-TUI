...
`,
  ...
  const pure = process.env.SMART_PURE
  const meta = process.env.SMART_PLUGIN_META_FILE
  process.env.SMART_PURE = "1"
  process.env.SMART_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.SMART_PURE
    } else {
      process.env.SMART_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.SMART_PLUGIN_META_FILE
    } else {
      process.env.SMART_PLUGIN_META_FILE = meta
    }
  }
})
