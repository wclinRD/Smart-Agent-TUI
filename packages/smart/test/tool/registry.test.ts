...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".smart")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .smart/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const smart = path.join(test.directory, ".smart")
      const tool = path.join(smart, "tool")
  ...
  it.instance("ignores non-tool exports in .smart/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".smart", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-smart` was registering a tool
  ...
  it.instance("loads tools from .smart/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const smart = path.join(test.directory, ".smart")
      const tools = path.join(smart, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".smart", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const smart = path.join(test.directory, ".smart")
        const customTools = path.join(smart, "tools")
        const plugin = path.join(smart, "node_modules", "@smart-ai", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(smart, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".smart", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(smart, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(smart, "node_modules", "cowsay")
