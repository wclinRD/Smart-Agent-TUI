...
import { AppProcess } from "@smart-ai/core/process"
...
// Typed equivalent of constructing argv for `smart run`. New flags should
...
// `smart serve` is a long-lived process — it never exits on its own.
...
// `smart acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `smart serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `smart acp` and return a duplex JSON-RPC handle. Long-lived:
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `smart serve did not become ready within ${readyTimeoutMs}ms\n` +
