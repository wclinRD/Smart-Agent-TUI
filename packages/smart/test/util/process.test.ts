...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.SMART_TEST ?? "")'), {
      env: {
        SMART_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "SMART_TEST_SHELL"], {
      shell: true,
      env: {
        SMART_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("SMART_TEST_SHELL=ok")
  })
