...
Создайте `.smart/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию smart или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем smart [config](/docs/config):
...
- Глобальный: `~/.config/smart/commands/`
- Для каждого проекта: `.smart/commands/`
...
smart включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
