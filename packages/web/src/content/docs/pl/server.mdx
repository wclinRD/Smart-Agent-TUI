---
title: Serwer
description: Interakcja z serwerem smart za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `smart serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient smart.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `smart.local` |
...
Ustaw `SMART_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `smart` lub ustaw `SMART_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `smart serve`, jak i `smart web`.
...
Kiedy uruchomisz `smart`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera smart do programowej interakcji z smart.
:::

Ta architektura umożliwia obsługę wielu klientów przez smart i programową interakcję z kodem otwartym.

Możesz uruchomić `smart serve`, aby uruchomić samodzielny serwer. Jeśli masz
smart TUI działa, `smart serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki smart [IDE](/docs/ide).
...
Serwer smart udostępnia następujące interfejsy API.
