...
Dzięki smart możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie smart używa naszego własnego motywu `smart`.
...
smart ma kilka wbudowanych motywów.
...
- Chcą, aby Smart pasował do wyglądu ich terminala
...
smart obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/smart/themes/*.json` lub `$XDG_CONFIG_HOME/smart/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.smart/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.smart/themes/*.json`
