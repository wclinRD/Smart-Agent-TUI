---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w smart.
---

import config from "../../../../config.mjs"
export const console = config.console

smart używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji smart.
...
w `~/.local/share/smart/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim smart
config.
...
Smart Zen to lista modeli dostarczonych przez zespół smart, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z smart. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Smart Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Smart Zen` i przejdź do [smart.ai/auth](https://smart.ai/zen).
...
Działa jak każdy inny dostawca w smart i jest całkowicie opcjonalny w użyciu.

---

## Smart Go

Smart Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół smart, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z smart.

1. Uruchom polecenie `/connect` w TUI, wybierz `Smart Go` i przejdź do [smart.ai/auth](https://smart.ai/zen).
...
Aby używać Amazon Bedrock z smart:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania smart:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `smart.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w smart nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować smart do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod smart działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji smart.
...
smart używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować smart
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`smart.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem smart:
...
Aby używać Google Vertex AI z smart:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania smart.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą smart, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [smart-helicone-session](https://github.com/H2Shami/smart-helicone-session), aby automatycznie rejestrować każdą konwersację smart jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja smart będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować smart tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować smart tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować smart tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem smart. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/smart).
:::
...
Aby korzystać z Ollama Cloud z smart:
...
3. Skopiuj klucz API do użycia w smart.
...
6. **Ważne**: Przed użyciem modeli chmurowych w smart musisz pobrać informacje o modelu lokalnie:
...
Smart Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół smart. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Smart Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Smart Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API smart.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji smart.
...
5. Możesz je także dostosować za pomocą konfiguracji smart. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z smart:
...
Możesz także dostosować modele za pomocą konfiguracji smart. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z smart. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `smart.json` w katalogu projektu:
...
Pola `limit` pozwalają smart zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `smart auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację smart i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji smart.
