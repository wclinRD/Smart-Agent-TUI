...
Umiejętności agenta pozwalają smart odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
smart przeszukuje te lokalizacje:

- Project config: `.smart/skills/<name>/SKILL.md`
- Global config: `~/.config/smart/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu smart przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.smart/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/smart/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.smart/skills/git-release/SKILL.md` w ten sposób:
...
smart wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `smart.json`:
...
**Dla agentów wbudowanych** (w `smart.json`):
