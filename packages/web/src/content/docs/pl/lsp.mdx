---
title: Serwery LSP
description: Smart integruje się z serwerami LSP.
---

Smart może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

Smart posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `SMART_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i Smart otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji Smart.
