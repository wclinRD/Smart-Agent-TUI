---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć smart.
---

Wtyczki umożliwiające wyłączenie smart poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania smart.
...
- `.smart/plugins/` - Wtyczki na poziomie projektu
- `~/.config/smart/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/smart/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/smart/smart.json`)
2. Konfiguracja projektu (`smart.json`)
3. Globalny katalog wtyczek (`~/.config/smart/plugins/`)
4. Katalog wtyczek projektu (`.smart/plugins/`)
...
smart uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z smart do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do smart.
...
:::note
Jeśli korzystasz z aplikacji komputerowej smart, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj smart czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do smart:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod smart. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://smart.ai/docs/sdk).
