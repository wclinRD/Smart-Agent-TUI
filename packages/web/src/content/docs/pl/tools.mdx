...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Smart posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Smart.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `SMART_EXPERIMENTAL_LSP_TOOL=true` (lub `SMART_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Smart lub gdy zmienna środowiskowa `SMART_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Smart:
