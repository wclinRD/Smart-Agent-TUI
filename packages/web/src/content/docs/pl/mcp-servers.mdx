...
Możesz dodać dodatkowe informacje do smart za pomocą _Model Context Protocol_, w skrócie MCP. smart obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [smart Config](https://smart.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/smart`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
smart automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, smart:
...
Jeśli serwer wymaga uwierzytelnienia, smart poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `smart mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji smart bezpieczne przechowa tokeny w `~/.local/share/smart/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w smart, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie smart, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć smart z kontem Sentry.
