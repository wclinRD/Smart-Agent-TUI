---
title: Provedores
description: Usando qualquer provedor LLM no smart.
---

import config from "../../../../config.mjs"
export const console = config.console

smart usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do smart.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/smart/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do smart.
...
Smart Zen é uma lista de modelos fornecidos pela equipe do smart que foram testados e verificados para funcionar bem com o smart. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o Smart Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `Smart Zen` e acesse [smart.ai/zen](https://smart.ai/zen).
...
Funciona como qualquer outro provedor no smart e é completamente opcional.

---

## Smart Go

Smart Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do smart que foram
testados e verificados para funcionar bem com o smart.

1. Execute o comando `/connect` no TUI, selecione `Smart Go` e acesse [smart.ai/zen](https://smart.ai/zen).
...
Para usar o Amazon Bedrock com o smart:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o smart:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `smart.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no smart não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o smart para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o smart funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do smart.
...
armazenar o token no armazenamento de autenticação do smart.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
smart usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o smart
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `smart.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `smart.json`:
...
Para usar sua assinatura do GitHub Copilot com o smart:
...
Para usar o Google Vertex AI com o smart:
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do smart, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [smart-helicone-session](https://github.com/H2Shami/smart-helicone-session) para registrar automaticamente cada conversa do smart como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do smart listada como uma sessão separada.
...
Você pode configurar o smart para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o smart para usar modelos locais através do LM Studio.
...
Você pode configurar o smart para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o smart. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/smart) para detalhes.
:::
...
Para usar o Ollama Cloud com o smart:
...
3. Copie a chave da API para uso no smart.
...
6. **Importante**: Antes de usar modelos em nuvem no smart, você deve puxar as informações do modelo localmente:
...
Smart Zen é uma lista de modelos testados e verificados fornecidos pela equipe do smart. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>Smart Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **Smart Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do smart.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do smart.
...
5. Você também pode personalizá-los através da sua configuração do smart. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o smart:
...
Você também pode personalizar modelos através da sua configuração do smart. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o smart. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no smart.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `smart.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o smart entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `smart auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do smart e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do smart.
