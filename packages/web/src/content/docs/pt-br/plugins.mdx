---
title: Plugins
description: Escreva seus próprios plugins para estender o smart.
---

Plugins permitem que você estenda o smart conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do smart.
...
- `.smart/plugins/` - Plugins em nível de projeto
- `~/.config/smart/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/smart/node_modules/`.
...
1. Configuração global (`~/.config/smart/smart.json`)
2. Configuração do projeto (`smart.json`)
3. Diretório de plugins global (`~/.config/smart/plugins/`)
4. Diretório de plugins do projeto (`.smart/plugins/`)
...
O smart executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do smart para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o smart.
...
:::note
Se você estiver usando o aplicativo desktop smart, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o smart de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao smart:
...
O helper `tool` cria uma ferramenta personalizada que o smart pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o smart junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://smart.ai/docs/sdk) para detalhes.
