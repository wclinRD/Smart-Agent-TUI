---
title: Servidor
description: Interaja com o servidor smart via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `smart serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente smart pode usar.
...
```bash
smart serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `smart.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
smart serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `SMART_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `smart`, ou defina `SMART_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `smart serve` quanto ao `smart web`.

```bash
SMART_SERVER_PASSWORD=your-password smart serve
```
...
Quando você executa `smart`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor smart para interagir com o smart programaticamente.
:::

Essa arquitetura permite que o smart suporte múltiplos clientes e permite que você interaja com o smart programaticamente.

Você pode executar `smart serve` para iniciar um servidor autônomo. Se você tiver o TUI do smart em execução, `smart serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do smart [IDE](/docs/ide).
...
O servidor smart expõe as seguintes APIs.
