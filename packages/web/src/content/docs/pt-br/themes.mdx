...
Com o smart, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o smart usa nosso próprio tema `smart`.
...
O smart vem com vários temas embutidos.
...
- Querem que o smart corresponda à aparência do seu terminal
...
O smart suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/smart/themes/*.json` ou `$XDG_CONFIG_HOME/smart/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.smart/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.smart/themes/*.json`
