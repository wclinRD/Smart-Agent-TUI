...
As habilidades do agente permitem que o smart descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O smart pesquisa nesses locais:

- Configuração do projeto: `.smart/skills/<name>/SKILL.md`
- Configuração global: `~/.config/smart/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o smart sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.smart/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/smart/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.smart/skills/git-release/SKILL.md` assim:
...
O smart lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `smart.json`:
...
**Para agentes embutidos** (em `smart.json`):
