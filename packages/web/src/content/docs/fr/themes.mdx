...
Avec Smart, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Smart utilise notre propre thème `smart`.
...
Smart est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Smart corresponde à l'apparence de leur terminal
...
Smart prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/smart/themes/*.json` ou `$XDG_CONFIG_HOME/smart/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.smart/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.smart/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/smart/themes
vim ~/.config/smart/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .smart/themes
vim .smart/themes/my-theme.json
```
...
{
  "$schema": "https://smart.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
