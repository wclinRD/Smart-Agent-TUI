...
Pour déboguer les problèmes avec Smart, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\smart\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `smart --log-level DEBUG`.

---

## Stockage

smart stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/smart/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\smart`
...
Smart Desktop exécute un serveur Smart local (le side-car `smart-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Smart` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/smart/smart.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\smart\smart.jsonc`
...
Smart peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/smart/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\smart\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Smart puisse le reconstruire.

1. Quittez complètement Smart Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/smart`
- **Linux** : supprimez `~/.cache/smart` (ou exécutez `rm -rf ~/.cache/smart`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\smart`

3. Redémarrez le bureau Smart.

---

### Résoudre les problèmes de connexion au serveur

Smart Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `smart.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `SMART_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `SMART_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, Smart Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.
...
Smart Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Smart dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Smart.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Smart Desktop) :

- `smart.settings.dat` (serveur de bureau par défaut URL)
- `smart.global.dat` et `smart.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Smart :
...
3. Assurez-vous d'avoir la dernière version avec `smart upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `smart models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\smart`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. smart installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\smart`

2. Redémarrez smart pour réinstaller les derniers packages du fournisseur

Cela forcera smart à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
smart détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
