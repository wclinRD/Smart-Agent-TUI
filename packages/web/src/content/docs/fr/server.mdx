---
title: Serveur
description: Interagissez avec le serveur smart via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `smart serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client smart peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `smart.local` |
...
Définissez `SMART_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `smart`, ou définissez `SMART_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `smart serve` et à `smart web`.
...
Lorsque vous exécutez `smart`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur smart pour interagir avec smart par programmation.
:::

Cette architecture permet à smart de prendre en charge plusieurs clients et vous permet d'interagir avec smart par programme.

Vous pouvez exécuter `smart serve` pour démarrer un serveur autonome. Si vous avez le
smart TUI en cours d'exécution, `smart serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Smart [IDE](/docs/ide).
...
Le serveur smart expose les API suivantes.
