---
title: Plugins
description: Écrivez vos propres plugins pour étendre Smart.
---

Les plugins vous permettent d'étendre Smart en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Smart.
...
- `.smart/plugins/` - Plugins au niveau du projet
- `~/.config/smart/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/smart/node_modules/`.
...
1. Configuration globale (`~/.config/smart/smart.json`)
2. Configuration du projet (`smart.json`)
3. Répertoire global des plugins (`~/.config/smart/plugins/`)
4. Répertoire des plugins du projet (`.smart/plugins/`)
...
Smart exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK smart pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre smart.
...
:::note
Si vous utilisez l'application de bureau Smart, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez smart de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à smart :
...
L'assistant `tool` crée un outil personnalisé que smart peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour smart aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://smart.ai/docs/sdk) pour plus de détails.
