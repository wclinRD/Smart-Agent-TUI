---
title: Servidor
description: Interactuar con el servidor smart a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `smart serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente smart puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `smart.local` |
...
Configure `SMART_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `smart`, o configure `SMART_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `smart serve` como a `smart web`.
...
Cuando ejecuta `smart`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor smart para interactuar con smart mediante programación.
:::

Esta arquitectura permite que smart admita múltiples clientes y le permite interactuar con smart mediante programación.

Puede ejecutar `smart serve` para iniciar un servidor independiente. Si tienes el
smart TUI ejecutándose, `smart serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Smart [IDE](/docs/ide).
...
El servidor smart expone las siguientes API.
