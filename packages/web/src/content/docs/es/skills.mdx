...
Las habilidades del agente permiten a Smart descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Smart busca estas ubicaciones:

- Configuración del proyecto: `.smart/skills/<name>/SKILL.md`
- Configuración global: `~/.config/smart/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Smart sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.smart/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/smart/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.smart/skills/git-release/SKILL.md` así:
...
Smart enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `smart.json`:
...
**Para agentes integrados** (en `smart.json`):
