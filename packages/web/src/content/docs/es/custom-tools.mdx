...
description: Cree herramientas que LLM pueda llamar en smart.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de smart como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.smart/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/smart/tools/`.
...
```ts title=".smart/tools/database.ts" {1}
...
```ts title=".smart/tools/project.ts" {8}
import { tool } from "@smart-ai/plugin"
...
```python title=".smart/tools/add.py"
import sys
...
```ts title=".smart/tools/python-add.ts" {10}
import { tool } from "@smart-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".smart/tools/add.py")
