...
Las herramientas permiten que LLM realice acciones en su código base. Smart viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Smart.
...
:::note
Esta herramienta solo está disponible cuando `SMART_EXPERIMENTAL_LSP_TOOL=true` (o `SMART_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Smart o cuando la variable de entorno `SMART_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Smart:

```bash
SMART_ENABLE_EXA=1 smart
```
