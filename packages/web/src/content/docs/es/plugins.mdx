---
title: Complementos
description: Escriba sus propios complementos para extender Smart.
---

Los complementos le permiten extender Smart al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Smart.
...
- `.smart/plugins/` - Complementos a nivel de proyecto
- `~/.config/smart/plugins/` - Complementos globales
...
```json title="smart.json"
{
  ...
  "plugin": ["smart-helicone-session", "smart-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/smart/node_modules/`.
...
1. Configuración global (`~/.config/smart/smart.json`)
2. Configuración del proyecto (`smart.json`)
3. Directorio global de complementos (`~/.config/smart/plugins/`)
4. Directorio de complementos del proyecto (`.smart/plugins/`)
...
Smart ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".smart/plugins/my-plugin.ts"
...
```js title=".smart/plugins/example.js"
...
- `client`: Un cliente SDK smart para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar smart.
...
```js title=".smart/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "smart"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Smart, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que smart lea archivos `.env`:

```javascript title=".smart/plugins/env-protection.js"
...
```javascript title=".smart/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a smart:

```ts title=".smart/plugins/custom-tools.ts"
import { type Plugin, tool } from "@smart-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que smart puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para smart junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://smart.ai/docs/sdk) para obtener más detalles.
...
```ts title=".smart/plugins/compaction.ts"
import type { Plugin } from "@smart-ai/plugin"
...
```ts title=".smart/plugins/custom-compaction.ts"
import type { Plugin } from "@smart-ai/plugin"
