...
Con Smart puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Smart usa nuestro propio tema `smart`.
...
Smart viene con varios temas integrados.
...
- Quiere que Smart coincida con la apariencia de su terminal
...
Smart admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/smart/themes/*.json` o `$XDG_CONFIG_HOME/smart/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.smart/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.smart/themes/*.json`
