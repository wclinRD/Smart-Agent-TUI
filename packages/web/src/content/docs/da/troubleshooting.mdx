...
For at fejlfinde problemer med Smart, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\smart\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `smart --log-level DEBUG`.

---

## Lagring

smart gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/smart/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\smart`
...
Smart Desktop kører en lokal Smart-server (`smart-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Smart`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/smart/smart.jsonc` (eller `~/.config/smart/smart.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/smart/smart.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\smart\smart.jsonc`
...
Smart kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/smart/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\smart\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Smart kan genopbygge den.

1. Afslut Smart Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/smart`
- **Linux**: slet `~/.cache/smart` (eller kør `rm -rf ~/.cache/smart`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\smart`

3. Start Smart Desktop på ny.

---

### Løs problemer med serverforbindelse

Smart Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `smart.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `SMART_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `SMART_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Smart Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Smart.

---

### Meddelelser vises ikke

Smart Desktop viser kun systemvarsler når:

- varsler er aktiveret for Smart i dine OS-indstillinger, og
...
1. Afslut Smart Desktop.
2. Find og slet disse filer (de findes i Smart Desktop-appens datamappe):

- `smart.settings.dat` (skrivebordsstandardserver URL)
- `smart.global.dat` og `smart.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Smart:
...
For at finde ud af hvilke modeller du har adgang til, kør `smart models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\smart`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. smart installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\smart`

2. Start smart på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge smart til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
smart vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
