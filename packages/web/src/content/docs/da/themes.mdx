...
Med Smart kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Smart vårt eget `smart`-tema.
...
Smart kommer med flere innebygde temaer.
...
- Vil at Smart skal matche terminalens utseende
...
Smart støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/smart/themes/*.json` eller `$XDG_CONFIG_HOME/smart/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.smart/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.smart/themes/*.json`
