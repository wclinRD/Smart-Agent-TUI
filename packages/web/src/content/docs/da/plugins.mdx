---
title: Plugins
description: Skriv dine egne plugins for at udvide Smart.
---

Plugins giver dig mulighed for at udvide Smart ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Smarts standardadfærd.
...
- `.smart/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/smart/node_modules/`.
...
1. Global konfiguration (`~/.config/smart/smart.json`)
2. Projektkonfiguration (`smart.json`)
3. Global plugin-mappe (`~/.config/smart/plugins/`)
4. Projekt plugin bibliotek (`.smart/plugins/`)
...
Smart kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En smart SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide smart.
...
:::note
Hvis du bruger Smart desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå smart i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til smart:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som smart kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for smart sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://smart.ai/docs/sdk) for detaljer.
