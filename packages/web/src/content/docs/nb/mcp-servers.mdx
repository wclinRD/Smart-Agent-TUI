...
Du kan legge til eksterne verktøy i Smart ved å bruke _Model Context Protocol_ eller MCP. Smart støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Smart Config](https://smart.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="smart.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/smart`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Smart håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Smart:
...
Hvis serveren krever autentisering, vil Smart be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `smart mcp auth <server-name>`.
...
```bash
smart mcp auth my-oauth-server
```
...
```bash
smart mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Smart lagre tokens sikkert i `~/.local/share/smart/mcp-auth.json`.
...
smart mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Smart, sammen med innebygde verktøy. Så du kan administrere dem gjennom Smart-konfigurasjonen som alle andre verktøy.
...
```json title="smart.json" {11, 14-18}
{
  "$schema": "https://smart.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
smart mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Smart til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Smart-samtale oppført som en separat økt.
