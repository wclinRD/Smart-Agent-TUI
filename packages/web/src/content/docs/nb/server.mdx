---
title: Tjener
description: Samhandle med Smart-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `smart serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Smart-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `smart.local` |
...
Sett `SMART_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `smart` som standard, eller sett `SMART_SERVER_USERNAME` for å overstyre det. Dette gjelder både `smart serve` og `smart web`.
...
Når du kjører `smart` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Smart-serveren til å samhandle med Smart programmatisk.
:::

Denne arkitekturen lar Smart støtte flere klienter og lar deg samhandle med Smart programmatisk.

Du kan kjøre `smart serve` for å starte en frittstående server. Hvis du har
Smart TUI kjører, vil `smart serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Smart [IDE](/docs/ide) plugins.
...
smart-serveren viser følgende APIer.
