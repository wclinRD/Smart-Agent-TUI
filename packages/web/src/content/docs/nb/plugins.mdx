...
description: Skriv dine egne programtillegg for å utvide Smart.
---

Plugins lar deg utvide Smart ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Smart.
...
- `.smart/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/smart/node_modules/`.
...
1. Global konfigurasjon (`~/.config/smart/smart.json`)
2. Prosjektkonfigurasjon (`smart.json`)
3. Global plugin-katalog (`~/.config/smart/plugins/`)
4. Prosjektpluginkatalog (`.smart/plugins/`)
...
Smart kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Smart SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Smart.
...
:::note
Hvis du bruker Smart-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Smart fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Smart:
...
`tool`-hjelperen lager et tilpasset verktøy som Smart kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Smart sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://smart.ai/docs/sdk) for detaljer.
