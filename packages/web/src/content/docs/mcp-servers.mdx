...
You can add external tools to Smart using the _Model Context Protocol_, or MCP. Smart supports both local and remote servers.
...
You can define MCP servers in your [Smart Config](https://smart.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/smart` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Smart automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Smart will:
...
If the server requires authentication, Smart will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `smart mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Smart will store the tokens securely in `~/.local/share/smart/mcp-auth.json`.
...
Your MCPs are available as tools in Smart, alongside built-in tools. So you can manage them through the Smart config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Smart to your Sentry account.
