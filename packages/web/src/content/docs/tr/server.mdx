---
title: Sunucu
description: smart sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`smart serve` komutu, smart istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `smart.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `SMART_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `smart` değeridir; değiştirmek isterseniz `SMART_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `smart serve` hem de `smart web` için geçerlidir.
...
`smart` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
smart ile programatik etkileşim için smart sunucusunu kullanın.
:::

Bu mimari, smart'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`smart serve` ile bağımsız bir sunucu başlatabilirsiniz. smart TUI açıksa, `smart serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum smart [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
smart sunucusu aşağıdaki API'leri sunar.
