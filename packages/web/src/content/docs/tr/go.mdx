...
Smart Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Smart'daki diğer sağlayıcılar gibi çalışır. Smart Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Smart'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Smart Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Smart Go, Smart'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Smart Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Smart Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Smart Go'ya abone olabilir.
:::
...
Smart Go aşağıdaki limitleri içerir:
...
Smart yapılandırmanızdaki [model id](/docs/config/#models) formatı `smart-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `smart-go/kimi-k2.7-code` kullanmalısınız.
...
Smart Go'yu şu amaçlarla oluşturduk:
...
4. Smart ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
