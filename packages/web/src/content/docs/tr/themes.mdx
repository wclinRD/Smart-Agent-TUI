...
smart ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak smart kendi `smart` temasını kullanır.
...
smart birden fazla yerleşik temayla gelir.
...
- smart'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
smart, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/smart/themes/*.json` veya `$XDG_CONFIG_HOME/smart/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.smart/themes/*.json`
4. **Geçerli çalışma dizini** - `./.smart/themes/*.json`
