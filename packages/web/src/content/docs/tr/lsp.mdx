...
description: Smart, LSP sunucularınızla bütünleşir.
---

Smart, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Smart, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`SMART_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve smart bir dosyayı açtığında:
...
LSP sunucularını, smart yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
