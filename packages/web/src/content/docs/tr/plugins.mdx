---
title: Eklentiler
description: Smart'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Smart'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Smart'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.smart/plugins/` - Proje düzeyinde eklentiler
- `~/.config/smart/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/smart/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/smart/smart.json`)
2. Project config (`smart.json`)
3. Global eklenti dizini (`~/.config/smart/plugins/`)
4. Proje eklenti dizini (`.smart/plugins/`)
...
Smart bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Smart SDK istemcisi.
...
Smart'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Smart masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Smart'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Smart'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Smart'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Smart'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://smart.ai/docs/sdk) bakın.
