...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. smart yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
smart içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `SMART_EXPERIMENTAL_LSP_TOOL=true` (veya `SMART_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca smart provider kullanılırken veya `SMART_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

smart başlatırken etkinleştirmek için:
