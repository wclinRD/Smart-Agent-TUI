...
_Model Bağlam Protokolü_ veya MCP kullanarak smart'a harici araçlar ekleyebilirsiniz. smart hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [smart Config](https://smart.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/smart` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
smart, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde smart şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa smart, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `smart mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından smart, belirteçleri `~/.local/share/smart/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra smart'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi smart yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve smart'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
