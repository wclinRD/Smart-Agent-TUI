...
Agent Skills erlauben Smart, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Smart sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Smart vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.smart/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/smart/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.smart/skills/git-release/SKILL.md` zum Beispiel so:
...
Smart listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `smart.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `smart.json`):
