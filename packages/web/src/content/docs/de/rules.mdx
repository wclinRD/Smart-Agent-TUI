---
title: Regeln
description: Lege eigene Anweisungen fuer smart fest.
---

Du kannst smart ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in smart den Befehl `/init` aus.
...
So kann smart sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht smart sie zu erweitern.
...
smart kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/smart/AGENTS.md` speichern.
Diese gelten in allen smart-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Smart dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/smart/AGENTS.md` existiert)
...
Beim Start sucht smart in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/smart/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `smart.json` oder in `~/.config/smart/smart.json` angeben.
...
smart parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit smart.json

Empfohlen ist das `instructions`-Feld in der `smart.json`:
...
Du kannst smart explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `smart.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
