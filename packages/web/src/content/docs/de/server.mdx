---
title: Server
description: Nutze den smart-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `smart serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein smart-Client nutzen kann.
...
Setze `SMART_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `smart`, kann aber mit `SMART_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `smart serve` und `smart web`.
...
Wenn du `smart` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den smart-Server, um smart programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `smart serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `smart serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Smart-[IDE](/docs/ide)-Plugins.
...
Der smart-Server stellt folgende APIs bereit.
