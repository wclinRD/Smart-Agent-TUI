...
Wenn Smart Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\smart\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `smart --log-level DEBUG`.

---

## Speicher

smart speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/smart/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\smart`
...
Smart Desktop startet im Hintergrund einen lokalen Smart-Server (`smart-cli`-Sidecar).
...
- Nur macOS: `Smart`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/smart/smart.jsonc` (oder `~/.config/smart/smart.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/smart/smart.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\smart\smart.jsonc`
...
Smart kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/smart/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\smart\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Smart ihn neu aufbauen kann.

1. Beende Smart Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/smart`
- **Linux**: loesche `~/.cache/smart` (oder fuehre aus: `rm -rf ~/.cache/smart`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\smart`

3. Starte Smart Desktop neu.

---

### Server-Verbindungsprobleme beheben

Smart Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `smart.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `SMART_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `SMART_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Smart Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Smart-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Smart Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Smart in den OS-Einstellungen aktiviert sind, und
...
1. Beende Smart Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Smart Desktop):

- `smart.settings.dat` (desktop default server URL)
- `smart.global.dat` und `smart.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Smart hast:
...
3. Stelle sicher, dass du die neueste Version hast: `smart upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `smart models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\smart`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. smart installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\smart`

2. Starte smart neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt smart, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
smart erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
