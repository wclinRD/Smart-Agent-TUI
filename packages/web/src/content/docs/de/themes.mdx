...
Mit Smart kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Smart das `smart`-Theme.
...
Smart liefert mehrere eingebaute Themes mit.
...
- Smart optisch ans Terminal anpassen wollen
...
Smart unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/smart/themes/*.json` oder `$XDG_CONFIG_HOME/smart/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.smart/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.smart/themes/*.json`
