...
Smart Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Smart. Du abonnierst Smart Go und
...
Smart Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Smart Go funktioniert wie jeder andere Provider in Smart.

1. Du meldest dich bei **<a href={console}>Smart Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Smart Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Smart Go abonnieren.
:::
...
Smart Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Smart Config verwendet das Format `smart-go/<model-id>`. Für Kimi K2.7 Code würdest du beispielsweise `smart-go/kimi-k2.7-code` in deiner Config verwenden.
...
Wir haben Smart Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Smart zu nutzen.
