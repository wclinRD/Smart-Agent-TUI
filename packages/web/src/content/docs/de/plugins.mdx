---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Smart zu erweitern.
---

Mit Plugins können Sie Smart erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Smart zu ändern.
...
- `.smart/plugins/` – Plugins auf Projektebene
- `~/.config/smart/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/smart/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/smart/smart.json`)
2. Projektkonfiguration (`smart.json`)
3. Globales Plugin-Verzeichnis (`~/.config/smart/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.smart/plugins/`)
...
Smart führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Smart-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Smart erweitern können.
...
:::note
Wenn Sie die Smart-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Smart `.env`-Dateien liest:
...
Plugins können Smart auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Smart aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Smart zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://smart.ai/docs/sdk).
