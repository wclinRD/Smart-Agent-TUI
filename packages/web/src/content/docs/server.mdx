---
title: Server
description: Interact with smart server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `smart serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an smart client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `smart.local` |
...
Set `SMART_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `smart`, or set `SMART_SERVER_USERNAME` to override it. This applies to both `smart serve` and `smart web`.
...
When you run `smart` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the smart server to interact with smart programmatically.
:::

This architecture lets smart support multiple clients and allows you to interact with smart programmatically.

You can run `smart serve` to start a standalone server. If you have the
smart TUI running, `smart serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Smart [IDE](/docs/ide) plugins.
...
The smart server exposes the following APIs.
