...
Da biste otklonili probleme s Smart, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\smart\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `smart --log-level DEBUG`.

---

## Pohrana

Smart pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/smart/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\smart`
...
Smart Desktop pokreće lokalni Smart server (`smart-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Smart` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/smart/smart.jsonc` (ili `~/.config/smart/smart.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/smart/smart.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\smart\smart.jsonc`
...
Smart također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/smart/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\smart\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Smart mogao ponovo izgraditi.

1. Potpuno zatvorite Smart Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/smart`
- **Linux**: obrišite `~/.cache/smart` (ili pokrenite `rm -rf ~/.cache/smart`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\smart`

3. Ponovo pokrenite Smart Desktop.

---

### Rješavanje problema sa vezom na serveru

Smart Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `smart.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `SMART_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `SMART_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Smart Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Smart karakteristikama.

---

### Obavještenja se ne prikazuju

Smart Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Smart u postavkama vašeg OS-a, i
...
1. Zatvorite Smart Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Smart Desktop aplikacije):

- `smart.settings.dat` (URL zadanog servera za desktop)
- `smart.global.dat` i `smart.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `smart upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `smart models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\smart`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Smart dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\smart`

2. Ponovo pokrenite Smart da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Smart da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Smart će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
