...
U Smart mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Smart koristi nasu `smart` temu.
...
Smart dolazi sa vise ugradenih tema.
...
- Zele da Smart odgovara izgledu njihovog terminala
...
Smart podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/smart/themes/*.json` ili `$XDG_CONFIG_HOME/smart/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.smart/themes/*.json`
4. **Trenutni radni direktorij** - `./.smart/themes/*.json`
