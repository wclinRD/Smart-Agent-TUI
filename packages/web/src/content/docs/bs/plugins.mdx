---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Smart.
---

Dodaci vam omogućavaju da proširite Smart spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Smart.
...
- `.smart/plugins/` - Dodaci na nivou projekta
- `~/.config/smart/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/smart/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/smart/smart.json`)
2. Konfiguracija projekta (`smart.json`)
3. Globalni direktorij dodataka (`~/.config/smart/plugins/`)
4. Direktorij dodataka projekta (`.smart/plugins/`)
...
Smart pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Smart.
...
Spriječite smart da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u smart:
...
Pomoćnik `tool` kreira prilagođeni alat koji smart može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za smart zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://smart.ai/docs/sdk) za detalje.
