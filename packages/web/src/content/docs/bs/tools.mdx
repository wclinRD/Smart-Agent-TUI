...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Smart dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Smart.
...
:::note
Ovaj alat je dostupan samo kada je `SMART_EXPERIMENTAL_LSP_TOOL=true` (ili `SMART_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Smart provajdera ili kada je varijabla `SMART_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Smart:
