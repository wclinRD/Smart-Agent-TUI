---
title: Server
description: Komunicirajte s smart serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `smart serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji smart klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `smart.local` |
...
Postavite `SMART_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `smart`, ili postavite `SMART_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `smart serve` i za `smart web`.
...
Kada pokrenete `smart`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite smart server za programsku interakciju sa smart-om.
:::

Ova arhitektura omogucava smart podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `smart serve` da startate standalone server. Ako je smart TUI vec pokrenut, `smart serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Smart [IDE](/docs/ide) pluginovi.
...
smart server izlaže sljedece API-je.
