---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Smart.
---

import config from "../../../../config.mjs"
export const console = config.console

Smart koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Smart konfiguraciji.
...
u `~/.local/share/smart/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Smart
config.
...
Smart Zen je lista modela koje je obezbedio Smart tim koji su bili
testirano i potvrđeno da dobro radi sa Smart. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Smart Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite smart i idite na [smart.ai/auth](https://smart.ai/auth).
...
Radi kao i svaki drugi provajder u Smart i potpuno je opcionalan za korištenje.

---

## Smart Go

Smart Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Smart tim i koji su testirani i verificirani da dobro rade s Smart-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Smart Go` i idite na [smart.ai/auth](https://smart.ai/zen).
...
Da biste koristili Amazon Bedrock s Smart:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete smart:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `smart.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Smart.
:::
...
Možete konfigurirati smart za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi smart ispravno radio.
   :::
...
Također možete dodati modele kroz svoju smart konfiguraciju.
...
da pohrani token u smart auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Smart koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Smart
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`smart.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Da biste koristili svoju GitHub Copilot pretplatu s smart:
...
Za korištenje Google Vertex AI s Smart:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće smart.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem smarta, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [smart-helicone-session](https://github.com/H2Shami/smart-helicone-session) da automatski prijavite svaki Smart razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Smart razgovor naveden kao zasebna sesija.
...
Možete konfigurirati smart za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati smart za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati smart za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Smart. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/smart) za detalje.
:::
...
Da biste koristili Ollama Cloud s Smart:
...
3. Kopirajte API ključ za korištenje u Smart.
...
6. **Važno**: Prije upotrebe modela oblaka u Smart, morate lokalno povući informacije o modelu:
...
Smart Zen je lista testiranih i verifikovanih modela koju je obezbedio Smart tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Smart Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Smart Zen**.

```txt
   /connect
```

3. Unesite svoj Smart API ključ.
...
Također možete dodati dodatne modele putem vaše smart konfiguracije.
...
5. Također ih možete prilagoditi putem vaše smart konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju smart konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s smart-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `smart.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Smart da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `smart auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Smart konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj smart konfiguraciji.
