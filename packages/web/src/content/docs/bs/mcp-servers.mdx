...
Možete dodati vanjske alate u Smart koristeći _Model Context Protocol_, ili MCP. Smart podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Smart Config](https://smart.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/smart`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Smart automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Smart će:
...
Ako server zahtijeva autentifikaciju, Smart će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `smart mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Smart sigurno čuva tokene u `~/.local/share/smart/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Smart, zajedno s ugrađenim alatima. Možete njima upravljati kroz Smart konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Smart sa vašim Sentry nalogom.
