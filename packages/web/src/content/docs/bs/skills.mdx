...
Agent skills omogucavaju Smart da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Smart pretrazuje ove lokacije:

- Konfiguracija projekta: `.smart/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/smart/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Smart ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.smart/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/smart/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.smart/skills/git-release/SKILL.md` ovako:
...
Smart navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `smart.json`:
...
**Za ugradene agente** (u `smart.json`):
