---
title: Konfiguracija
description: Korištenje Smart JSON konfiguracije.
---

Možete konfigurirati Smart koristeći JSON konfiguracijski fajl.

---

## Format

Smart podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/smart`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/smart/smart.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`SMART_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`smart.json` u projektu) - postavke specifične za projekat
5. **`.smart` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`SMART_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.smart` i `~/.config/smart` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/smart` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Smart konfiguraciju u `~/.config/smart/smart.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/smart/tui.json`.
...
Dodajte `smart.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Smart pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `SMART_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `SMART_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.smart` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.smart`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`smart.ai/config.json`**](https://smart.ai/config.json).

TUI konfiguracija koristi [**`smart.ai/tui.json`**](https://smart.ai/tui.json).
...
Koristite `SMART_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `smart.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `smart serve` i `smart web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Smart server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `smart.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Smart konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Smart pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/smart/agents/` ili `.smart/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Smart će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`smart run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/smart/commands/` ili `.smart/commands/`. [Saznajte više ovdje](/docs/commands).
...
Smart će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Smart **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Smart sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.smart/plugins/` ili `~/.config/smart/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Smart da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
