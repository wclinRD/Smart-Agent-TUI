...
With Smart you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Smart uses our own `smart` theme.
...
Smart comes with several built-in themes.
...
- Want Smart to match their terminal's appearance
...
Smart supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/smart/themes/*.json` or `$XDG_CONFIG_HOME/smart/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.smart/themes/*.json`
4. **Current working directory** - Defined in `./.smart/themes/*.json`
