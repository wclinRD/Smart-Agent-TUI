...
تتيح مهارات الوكيل لـ Smart اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Smart في هذه المواقع:

- إعدادات المشروع: `.smart/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/smart/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Smart صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.smart/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/smart/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.smart/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Smart المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `smart.json`:
...
**للوكلاء المدمجين** (في `smart.json`):
