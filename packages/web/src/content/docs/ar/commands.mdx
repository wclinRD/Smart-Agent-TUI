...
أنشئ الملف `.smart/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Smart أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Smart:
...
- على مستوى النظام: `~/.config/smart/commands/`
- لكل مشروع: `.smart/commands/`
...
يتضمن smart عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
