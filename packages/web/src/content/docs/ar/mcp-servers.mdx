...
يمكنك إضافة أدوات خارجية إلى Smart باستخدام _Model Context Protocol_، أو MCP. يدعم Smart خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Smart](https://smart.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/smart` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Smart تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Smart بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Smart المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `smart mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Smart الرموز بشكل آمن في `~/.local/share/smart/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Smart إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Smart مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Smart بحساب Sentry الخاص بك.
