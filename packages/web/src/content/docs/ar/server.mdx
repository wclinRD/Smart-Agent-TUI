---
title: الخادم
description: تفاعل مع خادم smart عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `smart serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل smart استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `smart.local` |
...
عيّن `SMART_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `smart`، أو عيّن `SMART_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `smart serve` و `smart web`.
...
عند تشغيل `smart` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم smart للتفاعل مع smart برمجيا.
:::

تتيح هذه البنية لـ smart دعم عدة عملاء وتمكّنك من التفاعل مع smart برمجيا.

يمكنك تشغيل `smart serve` لبدء خادم مستقل. إذا كانت واجهة smart في terminal (TUI)
قيد التشغيل، فسيبدأ `smart serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Smart لـ [IDE](/docs/ide).
...
يعرض خادم smart واجهات API التالية.
