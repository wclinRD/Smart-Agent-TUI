...
مع Smart يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Smart سمتنا `smart`.
...
- يريدون أن يطابق Smart مظهر terminal لديهم
...
يدعم Smart نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/smart/themes/*.json` أو `$XDG_CONFIG_HOME/smart/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.smart/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.smart/themes/*.json`
