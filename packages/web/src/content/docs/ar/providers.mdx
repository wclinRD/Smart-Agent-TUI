---
title: المزوّدون
description: استخدام أي موفّر LLM في Smart.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Smart ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Smart.
...
في `~/.local/share/smart/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Smart.
...
Smart Zen هي قائمة نماذج يوفّرها فريق Smart وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Smart. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Smart Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر smart، ثم انتقل إلى [smart.ai/auth](https://smart.ai/auth).
...
يعمل مثل أي مزوّد آخر في Smart واستخدامه اختياري بالكامل.

---

## Smart Go

Smart Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Smart، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Smart.

1. شغّل الأمر `/connect` في TUI، واختر `Smart Go`، ثم انتقل إلى [smart.ai/auth](https://smart.ai/zen).
...
لاستخدام Amazon Bedrock مع Smart:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل smart:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `smart.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Smart غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين smart لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل smart بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات smart.
...
تخزين الرمز في مساحة تخزين مصادقة smart.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Smart نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Smart
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`smart.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع smart:
...
لاستخدام Google Vertex AI مع Smart:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر smart، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [smart-helicone-session](https://github.com/H2Shami/smart-helicone-session) لتسجيل كل محادثة Smart تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Smart مدرجة كجلسة مستقلة.
...
يمكنك ضبط smart لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط smart لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط smart لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Smart. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/smart) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Smart:
...
3. انسخ مفتاح API لاستخدامه في Smart.
...
6. **مهم**: قبل استخدام نماذج السحابة في Smart، يجب سحب معلومات النموذج محليا:
...
Smart Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Smart. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Smart Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Smart Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Smart.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات smart.
...
5. يمكنك أيضا تخصيصها عبر إعدادات smart. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات smart. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع smart. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `smart.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Smart بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `smart auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات smart وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات smart.
