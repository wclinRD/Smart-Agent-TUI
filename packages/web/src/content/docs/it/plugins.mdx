---
title: Plugin
description: Scrivi plugin per estendere Smart.
---

I plugin ti permettono di estendere Smart agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Smart.
...
- `.smart/plugins/` - plugin a livello progetto
- `~/.config/smart/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/smart/node_modules/`.
...
1. Config globale (`~/.config/smart/smart.json`)
2. Config di progetto (`smart.json`)
3. Directory plugin globale (`~/.config/smart/plugins/`)
4. Directory plugin di progetto (`.smart/plugins/`)
...
Smart esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di smart per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere smart.
...
:::note
Se usi l'app desktop di Smart, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a smart di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a smart:
...
L'helper `tool` crea uno strumento personalizzato che smart può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in smart insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://smart.ai/docs/sdk) per i dettagli.
