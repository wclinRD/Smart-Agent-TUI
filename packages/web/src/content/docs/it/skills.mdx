...
Le skill degli agenti permettono a Smart di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Smart cerca in queste posizioni:

- Config di progetto: `.smart/skills/<name>/SKILL.md`
- Config globale: `~/.config/smart/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Smart risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.smart/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/smart/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.smart/skills/git-release/SKILL.md` cosi':
...
Smart elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `smart.json`:
...
**Per agenti integrati** (in `smart.json`):
