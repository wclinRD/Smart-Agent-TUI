...
Con Smart puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, Smart usa il tema `smart`.
...
Smart include diversi temi integrati.
...
- Vuole che Smart corrisponda all'aspetto del terminale
...
Smart supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/smart/themes/*.json` o `$XDG_CONFIG_HOME/smart/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.smart/themes/*.json`
4. **Directory di lavoro corrente** - in `./.smart/themes/*.json`
