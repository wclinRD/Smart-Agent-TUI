---
title: Supporto ACP
description: Usa Smart in qualunque editor compatibile con ACP.
---

Smart supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare Smart via ACP, configura l'editor in modo che esegua il comando `smart acp`.

Il comando avvia Smart come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'Smart' nel selettore agenti di AI Chat.
...
Per usare Smart come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare Smart come agente ACP per la chat.

Se devi passare variabili d'ambiente (come `SMART_API_KEY`), vedi [Configuring Adapters: Environment Variables](https://codecompanion.olimorris.dev/getting-started#setting-an-api-key) nella documentazione di CodeCompanion.nvim.

## Supporto

Smart funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione Smart
