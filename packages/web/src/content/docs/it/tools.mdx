...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. Smart include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in Smart.
...
:::note
Questo strumento e' disponibile solo quando `SMART_EXPERIMENTAL_LSP_TOOL=true` (oppure `SMART_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider Smart o quando la variabile d'ambiente `SMART_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di Smart:
