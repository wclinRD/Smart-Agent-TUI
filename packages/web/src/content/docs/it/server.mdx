---
title: Server
description: Interagisci con il server smart via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `smart serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client smart.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `smart.local` |
...
Imposta `SMART_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `smart`, oppure imposta `SMART_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `smart serve` sia per `smart web`.
...
Quando esegui `smart` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server smart per interagire con smart in modo programmatico.
:::

Questa architettura permette a smart di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `smart serve` per avviare un server standalone. Se la TUI di smart e gia in esecuzione, `smart serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Smart.
...
Il server smart espone le seguenti API.
