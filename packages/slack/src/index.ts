...
import { createOpencode, type ToolPart } from "@smartcode-ai/sdk"
...
console.log("🚀 Starting smart server...")
const smart = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await smart.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new smart session...")
    const { client, server } = smart
    ...
    console.log("✅ Created smart session:", createResult.data.id)
  ...
  console.log("📝 Sending to smart:", message.text)
