...
export async function spawnWslSidecar(
  ...
  const smart = await resolveWslOpencode(distro)
  if (!smart) throw new Error(`Smart is not installed in ${distro}`)
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export SMART_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export SMART_CLIENT=desktop",
    `export SMART_SERVER_USERNAME=${shellEscape(username)}`,
    `export SMART_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(smart)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
