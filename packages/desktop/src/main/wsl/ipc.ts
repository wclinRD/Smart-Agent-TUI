...
export function registerWslIpcHandlers(controller: WslServersController) {
  ...
  ipcMain.handle("wsl-servers-probe-smart", (_event: IpcMainInvokeEvent, name: string) =>
    controller.probeOpencode(requireWslIpcString("distro", name)),
  )
  ipcMain.handle("wsl-servers-install-smart", (_event: IpcMainInvokeEvent, name: string) =>
...
function registerUnavailableWslIpcHandlers() {
  ...
  ipcMain.handle("wsl-servers-probe-smart", unavailable)
  ipcMain.handle("wsl-servers-install-smart", unavailable)
