...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve smart route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "smart")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "smart")).toBe("openai")
    expect(statProvider("big-pickle", "", "smart")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "smart"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep smart as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "smart"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "smart"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep smart or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "smart"), country: "US" })).toMatchObject([
