...
// terminal sizes. The default smart tui command is excluded —
// `smartcode --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `smartcode --help` prints today;
...
// top-level help on `--help` and exits 1; not a real smart command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("smart CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ smart }) =>
      Effect.gen(function* () {
        const topLevel = yield* smart.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* smart.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`smartcode ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`smartcode ${argv.join(" ")} --help`)
        }
