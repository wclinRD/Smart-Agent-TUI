// Subprocess test harness for the smart CLI. Spawns the real binary against
...
//
// Configuration flows through smart's built-in test affordances:
//   - SMART_CONFIG_CONTENT      : provider config inline, no files to find
//   - SMART_TEST_HOME           : pins os.homedir() → tmpdir
//   - SMART_DISABLE_PROJECT_CONFIG : skip walking up for smart.json
//   - SMART_PURE                : skip external plugin discovery + install
//   - SMART_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `smart.run` is fully wired. The shape supports adding more
// builders (`smart.serve(opts)`, `smart.acp(opts)`, `smart.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of OpencodeCli.
import { test, type TestOptions } from "bun:test"
import { FSUtil } from "@smartcode-ai/core/fs-util"
import { AppProcess } from "@smartcode-ai/core/process"
...
const smartRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(smartRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    SMART_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    SMART_CONFIG_CONTENT: configJson,
    SMART_DISABLE_PROJECT_CONFIG: "1",
    SMART_PURE: "1",
    SMART_DISABLE_AUTOUPDATE: "1",
    SMART_DISABLE_AUTOCOMPACT: "1",
    SMART_DISABLE_MODELS_FETCH: "1",
    SMART_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `smartcode run`. New flags should
...
// `smartcode serve` is a long-lived process — it never exits on its own.
...
// `smartcode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `smartcode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `smartcode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("smart.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("smart.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "smart server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `smartcode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("smart.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const smart: OpencodeCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, smart })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `smart.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "smart") {
...
// every call site. Use this for any test that needs the smart CLI fixture.
//
...
// resources (e.g. `smart.serve`) without an extra `Effect.scoped` wrapper —
