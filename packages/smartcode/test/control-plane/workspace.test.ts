...
import { EventSequenceTable } from "@smartcode-ai/core/event/sql"
...
import { WorkspaceTable } from "@smartcode-ai/core/control-plane/workspace.sql"
...
import { Ripgrep } from "@smartcode-ai/core/ripgrep"

const originalEnv = {
  SMART_AUTH_CONTENT: process.env.SMART_AUTH_CONTENT,
  SMART_EXPERIMENTAL_WORKSPACES: process.env.SMART_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.SMART_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@smart.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.SMART_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=smart-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.SMART_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.SMART_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.SMART_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=smart-test")
