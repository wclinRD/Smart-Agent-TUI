...
it.instance("plan agent denies edits except .smart/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".smart/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".smart", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.SMART_TEST_HOME
      process.env.SMART_TEST_HOME = test.directory
