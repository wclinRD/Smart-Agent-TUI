...
import { SessionV1 } from "@smartcode-ai/core/v1/session"
import { FSUtil } from "@smartcode-ai/core/fs-util"
import { ModelsDev } from "@smartcode-ai/core/models-dev"
...
import { LLMEvent, LLMResponse } from "@smartcode-ai/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@smartcode-ai/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.smart.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.SMART_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["smart", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("SMART_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("SMART_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["smart", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "smart-proxy",
    name: "Smart proxy",
    providerID: ProviderV2.ID.smart,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["smart", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.SMART_RECORD_CONSOLE_TOKEN && process.env.SMART_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.smart,
        name: "Smart Zen",
        env: ["SMART_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.SMART_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.SMART_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.SMART_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["smart", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("SMART_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("SMART_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("SMART_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set SMART_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the smart LLM stack remain real.
  const metadata = {
