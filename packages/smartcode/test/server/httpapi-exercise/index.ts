 ...
 *
 * The script intentionally isolates `SMART_DB` before importing modules that touch
...
const scenarios: Scenario[] = [
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-smart-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-smart-directory")
  }),
  ...
  http.protected
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "smart", model: "test" })}`,
      headers: ctx.headers(),
    }))
    .json(200, array, "status"),
  ...
  http.protected
    ...
    .at((ctx) => ({
      ...
      headers: { ...ctx.headers(), "x-smart-ticket": "1" },
    }))
    .json(404, object, "status"),
  http.protected
