...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.SMART_WEBSEARCH_PROVIDER

    try {
      process.env.SMART_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.SMART_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.SMART_WEBSEARCH_PROVIDER
      else process.env.SMART_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for smart or explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderV2.ID.smart, { exa: false, parallel: false })).toBe(true)
