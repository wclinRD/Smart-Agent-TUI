...
import type { LLMClientShape } from "@smartcode-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("smart"))
    return { type: "unsupported", reason: "provider is not openai, smart, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @smartcode-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains smart-owned. The native runtime only adapts
      // the @smartcode-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
