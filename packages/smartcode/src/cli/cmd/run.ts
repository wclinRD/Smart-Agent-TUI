import type { PermissionV1 } from "@smartcode-ai/core/v1/permission"
// CLI entry point for `smartcode run`.
//
// Handles three modes:
...
//      smart server and runs interactive mode against it.
//
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@smartcode-ai/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run smart with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running smart server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to SMART_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to SMART_SERVER_USERNAME or 'smart')",
      })
      .option("dir", {
        type: "string",
