...
import type { Message, OpencodeClient, SessionMessageResponse } from "@smartcode-ai/sdk/v2"
...
export const AuthMethodID = "smart-login"
...
export class Service extends Context.Service<Service, Interface>()("@smartcode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `smartcode auth login` in the terminal",
      name: "Login with smart",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, smart provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const smartProvider = providers[ProviderV2.ID.make("smart")]
  const smartModel = smartProvider ? Provider.sort(Object.values(smartProvider.models))[0] : undefined
  if (smartProvider && smartModel) return { providerID: smartProvider.id, modelID: smartModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Smart service failure", service })
}
