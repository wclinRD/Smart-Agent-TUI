...
export const DEPRECATED_PLUGIN_PACKAGES = ["smart-openai-codex-auth", "smart-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, smartVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(smartVersion) || semver.major(smartVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.smart
  if (typeof range !== "string") return
  if (!semver.satisfies(smartVersion, range)) {
    throw new Error(`Plugin requires smart ${range} but running ${smartVersion}`)
  }
}
