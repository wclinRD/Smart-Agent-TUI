...
import { isRecord } from "@smartcode-ai/tui/util/record"
...
import { TuiKeybind } from "@smartcode-ai/tui/config/keybind"
...
export class Service extends Context.Service<Service, Interface>()("@smartcode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old smartcode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.smart`
  // folders between cwd and home, and SMART_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.SMART_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit SMART_TUI_CONFIG override, if set.
  if (Flag.SMART_TUI_CONFIG) {
    const configFile = Flag.SMART_TUI_CONFIG
  ...
  // 4. `.smart` directories (and SMART_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".smart") || dir.endsWith(".smartcode") || dir === Flag.SMART_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".smart") && !dir.endsWith(".smartcode") && dir !== Flag.SMART_CONFIG_DIR) continue
