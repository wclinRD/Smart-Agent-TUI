...
- Author package entrypoint is `@smartcode-ai/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/smart-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@smartcode-ai/plugin/tui`.
- `@smartcode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@smartcode-ai/plugin/tui"
...
- The value is a semver range checked against the running Smart version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.smart` is absent, no check is performed (backward compatible).
...
- `smartcode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `smartcode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.smart` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.smart`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.smart`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.smart` dir or `SMART_CONFIG_DIR`, Smart installs `@smartcode-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@smartcode-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Smart's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Smart registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Smart pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@smartcode-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"smart"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `smart.default`.
...
- Local plugins persist installed themes under the local `.smart/themes` area near the plugin config source.
...
- `--pure` / `SMART_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.smart/plugins/tui-smoke.tsx`
- Local vim plugin: `.smart/plugins/tui-vim.tsx`
- Local smoke config: `.smart/tui.json`
- Local smoke theme: `.smart/plugins/smoke-theme.json`
