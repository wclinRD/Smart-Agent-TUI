...
Practical reference for a future `packages/server` split after the smart
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/smartcode`.
...
Until enough shared service code lives outside `packages/smartcode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/smartcode`

It should not import `packages/smartcode` services while `packages/smartcode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on smart-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/smartcode` implementation details.
