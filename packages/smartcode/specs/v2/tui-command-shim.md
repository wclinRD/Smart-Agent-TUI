...
Delete `packages/smartcode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/smartcode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/smartcode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/smartcode`
- TUI plugin loader tests in `packages/smartcode` if runtime plugin API wiring changed
