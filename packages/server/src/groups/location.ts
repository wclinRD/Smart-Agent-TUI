...
function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-smart-workspace"]
  const directory =
    query.get("location[directory]") ||
    (request.headers["x-smart-directory"] ? decode(request.headers["x-smart-directory"]) : process.cwd())
