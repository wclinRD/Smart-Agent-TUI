...
describe("OpencodePlugin", () => {
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ SMART_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        expect((yield* catalog.provider.get(ProviderV2.ID.smart)).request.body.apiKey).toBe("public")
        expect((yield* catalog.model.get(ProviderV2.ID.smart, ModelV2.ID.make("paid"))).enabled).toBe(false)
      }),
    ),
  )
  ...
  it.effect("keeps free models without credentials", () =>
    withEnv({ SMART_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        expect((yield* catalog.provider.get(ProviderV2.ID.smart)).request.body.apiKey).toBe("public")
        expect((yield* catalog.model.get(ProviderV2.ID.smart, ModelV2.ID.make("free"))).enabled).toBe(true)
      }),
    ),
  )
  ...
  it.effect("treats output-only cost as free without credentials", () =>
    withEnv({ SMART_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        expect((yield* catalog.provider.get(ProviderV2.ID.smart)).request.body.apiKey).toBe("public")
        expect((yield* catalog.model.get(ProviderV2.ID.smart, ModelV2.ID.make("output-only"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("uses SMART_API_KEY as credentials", () =>
    withEnv({ SMART_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        const catalog = yield* Catalog.Service
        yield* plugin.add(pluginWithIntegrations(yield* Integration.Service))
        ...
        expect((yield* catalog.provider.get(ProviderV2.ID.smart)).request.body.apiKey).toBeUndefined()
        expect((yield* catalog.model.get(ProviderV2.ID.smart, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ SMART_API_KEY: undefined, CUSTOM_SMART_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        const catalog = yield* Catalog.Service
        ...
        yield* integrations.update((editor) => {
          editor.method.update({
            integrationID: Integration.ID.make("smart"),
            method: { type: "env", names: ["CUSTOM_SMART_API_KEY"] },
          })
        })
        const transform = yield* catalog.transform()
        yield* transform((catalog) => {
  ...
  it.effect("uses configured apiKey as credentials", () =>
    withEnv({ SMART_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        expect((yield* catalog.provider.get(ProviderV2.ID.smart)).request.body.apiKey).toBe("configured")
        expect((yield* catalog.model.get(ProviderV2.ID.smart, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("ignores non-smart providers and models", () =>
    withEnv({ SMART_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        const catalog = yield* Catalog.Service
  ...
  it.effect("prefers gpt-5-nano as the smart small model", () =>
    Effect.gen(function* () {
      const catalog = yield* Catalog.Service
      const providerID = ProviderV2.ID.smart
