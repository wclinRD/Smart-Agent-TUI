...
import { ModelsDevPlugin } from "@smartcode-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.SMART_MODELS_PATH,
          disabled: Flag.SMART_DISABLE_MODELS_FETCH,
        }
        Flag.SMART_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.SMART_MODELS_PATH = previous.path
          Flag.SMART_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
