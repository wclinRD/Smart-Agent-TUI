...
import { ConfigProvider } from "@smartcode-ai/core/config/provider"
import { ConfigMigrateV1 } from "@smartcode-ai/core/v1/config/migrate"
...
import { Policy } from "@smartcode-ai/core/policy"
...
describe("Config", () => {
  ...
  it.live("loads global, ancestor, and .smart configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
        ...
        return Effect.gen(function* () {
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".smart")),
              AbsolutePath.make(path.join(directory, ".smart")),
            ])
