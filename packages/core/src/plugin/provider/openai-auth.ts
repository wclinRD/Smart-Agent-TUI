...
function headers(contentType: string) {
  return { "Content-Type": contentType, "User-Agent": `smart/${InstallationVersion}` }
}
...
const successPage =
  "<!doctype html><title>Smart</title><h1>Authorization successful</h1><p>You can close this window.</p>"
const errorPage = (message: string) =>
  `<!doctype html><title>Smart</title><h1>Authorization failed</h1><p>${message.replace(/[&<>"']/g, "")}</p>`
