<!--
  ...
  and CUSTOMIZE_SMART_SKILL_DESCRIPTION). The body below becomes the
  skill's content.
-->

# Customizing smart

smart validates its own config strictly and refuses to start when a field
...
| Global skills                 | `~/.config/smart/skill(s)/<name>/SKILL.md`                                                                             |
...
```json
{
  ...
  "skills": {
    "paths": [".smart/skills", "/abs/path/to/skills"],
...
smart's skill loader scans for `**/SKILL.md` inside skill directories. The
file is named `SKILL.md` exactly, and lives in its own folder named after the
skill:

```
.smart/skills/my-skill/SKILL.md
```
...
```
.smart/agent/my-reviewer.md      OR     .smart/agents/my-reviewer.md
```
...
smart ships with `build`, `plan`, `general`, `explore`. Hidden internal agents:
...
```json
"plugin": [
  "smart-gemini-auth",            // npm spec, latest
  "smart-foo@1.2.3",              // npm spec, pinned
  "./local-plugin.ts",               // file path, relative to the declaring config
  "file:///abs/path/plugin.js",      // file URL
  ["smart-bar", { "key": "val" }] // tuple form with options
]
```

Auto-discovered plugins (no config entry needed): any `*.ts` or `*.js` file in
`.smart/plugin/` or `.smart/plugins/`.
...
smart evaluates the LAST matching rule, so put broad rules first and narrow
rules last.
...
- `SMART_CONFIG_CONTENT='{"$schema":"https://smart.ai/config.json"}'`:
  inject inline JSON as a final local-scope merge.
- `SMART_DISABLE_DEFAULT_PLUGINS=1`: skip default plugins.
- `SMART_PURE=1`: skip external plugins entirely.
- `SMART_DISABLE_EXTERNAL_SKILLS=1`,
  `SMART_DISABLE_CLAUDE_CODE_SKILLS=1`: skip the external skill scans under
  `~/.claude/` and `~/.agents/`.
...
- If the user's existing config is malformed, point them at the env-var escape
  ...
  session.
- After saving any config change, remind the user to quit and restart smart
