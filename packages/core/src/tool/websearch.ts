...
export class ConfigService extends Context.Service<ConfigService, Config>()("@smartcode/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.SMART_WEBSEARCH_PROVIDER === "exa" || process.env.SMART_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.SMART_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("SMART_EXPERIMENTAL") || truthy("SMART_ENABLE_EXA") || truthy("SMART_EXPERIMENTAL_EXA"),
    enableParallel: truthy("SMART_ENABLE_PARALLEL") || truthy("SMART_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `smart/${InstallationVersion}`,
