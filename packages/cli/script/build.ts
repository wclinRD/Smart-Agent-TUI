...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      SMART_CLI_NAME: `'${binary}'`,
      SMART_MODELS_DEV: modelsData,
      SMART_CHANNEL: `'${Script.channel}'`,
      SMART_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
  ...
  await Bun.write(
    ...
    JSON.stringify(
      {
        name: `@smartcode-ai/${name}`,
        version: Script.version,
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/anomalyco/smart.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
