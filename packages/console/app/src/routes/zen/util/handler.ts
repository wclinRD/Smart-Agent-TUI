...
import { and, Database, eq, isNull, lt, or, sql } from "@smartcode-ai/console-core/drizzle/index.js"
import { KeyTable } from "@smartcode-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@smartcode-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@smartcode-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@smartcode-ai/console-core/util/date.js"
...
import { ProviderTable } from "@smartcode-ai/console-core/schema/provider.sql.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-smart-session") ?? ""
    const requestId = input.request.headers.get("x-smart-request") ?? ""
    const ocClient = input.request.headers.get("x-smart-client") ?? ""
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWith429Retry(reqUrl, {
        ...
        headers: (() => {
          ...
          headers.delete("x-smart-request")
          headers.delete("x-smart-session")
          headers.delete("x-smart-project")
          headers.delete("x-smart-client")
          return headers
        })(),
        body: reqBody,
      })
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://smart.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://smart.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://smart.ai/workspace/${authInfo.workspaceID}/members`
