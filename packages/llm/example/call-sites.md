...
Kit and Aidan want provider-specific LLM behavior to move out of smart's AI
...
smart's dynamic runtime should construct executable models at its app
...
- Effect AI: executable models carry provider requirements and can be resolved by
  an app boundary.
- LiteLLM/smart config: dynamic `providerID/modelID` branching belongs at the
...
- [ ] Update tests so direct route/provider tests assert route values are carried
      by executable models, and smart/native tests assert boundary-based route
      selection.
