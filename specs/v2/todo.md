...
The smart server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@smartcode-ai/llm` tool loop after replacing its
...
- plugins get global "smart" instance like in that post i showed
- smart instance has stuff like `smart.session.prompt()` or
  `smart.tool.register({...})`
...
- decide whether to preserve deprecated `@smartcode-ai/llm` orchestration exports
