...
Both `action` and `resource` use smart's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.smart` files is intentionally deferred until `.smart` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether smart permits an operation through its managed execution paths.
