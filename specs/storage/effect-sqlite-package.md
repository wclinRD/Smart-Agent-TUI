...
The smart wrapper can implement that using Effect context instead of `LocalContext`:
...
Do not remove this behavior while moving smart to Effect SQLite. `SyncEvent.run` depends on transaction composability and `behavior: "immediate"` for sequencing correctness.
...
8. Only then build domain stores like session/message/project stores on top of smart's storage wrapper.
...
An Effect Drizzle SQLite package lets us vendor the adapter once. Then smart can build its own storage wrapper on top, and `SessionStorage`, `MessageStorage`, event store, and projector writes can all share the same transaction and migration model.
